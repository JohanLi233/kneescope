...
> **v2 → v3: the tracked gaps are closed.**
...
>    (spike at step ≈600, NaN by ≈1000, paired control clean on identical
...
>    first log-σ bin whose excess persistence (median ρ − median floor)
>    reaches ≥ 0.9 — and all σ*_upper values are recomputed under it
...
> 768×768 group was numerically invalid (uniform-grid CDF integration
> diverges under the c = 1 MP density singularity at y = 0; on synthetic
> pure-MP data the same code reports D ≈ 0.60). The rejection of MP
...
from random-matrix theory (knee = Marchenko–Pastur bulk edge λ₊) fails on
real training momentum: spectra are smooth heavy-tailed (Zipf slopes
−0.7…−2.7), entry noise is anisotropic at ratios of 10²–10⁴, and spike
peeling diverges. Second, we show the question can still be answered
assumption-free: cross-microbatch direction persistence ρ(σ) collapses
sharply at a measurable boundary σ*, stationary across our training window
(micro-steps 300→1900). Third, we state and validate the resulting law: σ*
is the *lower* edge of a transition band whose persistent-signal fraction
climbs from ≈0.3 to ≈1; the quality-optimal knee sits at the band's *upper*
...
dense model σ* is ~10× lower than on a 1080M MoE, and the warmup-tier
...
optimum sits at the band's upper edge (knee50 ≈ 0.01) on *both*
...
rule (per-shape knees at σ*, "EdgeCubic") is neutral at 500 steps but
...
orthogonalization of its momentum M = UΣVᵀ. The polar factor UVᵀ is never
computed exactly: a fixed Newton–Schulz polynomial iteration reshapes the
singular values through a scalar response τ(σ). "How Much Orthogonalization
...
t*(σ) under a spiked model (§3) — and *where it breaks*: real momentum is
...
on the naive reading of the law ("EdgeCubic", per-shape knees at σ*) fails
...
1. A snapshot-and-persistence measurement protocol (momentum + micro-batch
   gradients → ρ(σ) → per-shape-group σ*), with the analysis chain validated
   on synthetic spiked-MP ground truth (λ̂₊ within 2%, spike ρ 0.91 vs.
...
Orth(M) ≈ NS(M). Every published variant is a spectral map — singular
vectors kept, spectrum reshaped by a scalar response:

| variant | response | GEMMs/matrix | knee50 |
|---|---|---|---|
| classic NS5 (quintic, 5 steps) | band [0.65, 1.20], τ(1e-3)=0.47 | 15 | ≈0.001 |
| Polar Express (PE5) | band [0.80, 1.13], τ(1e-3)=0.86 | 15 | ≲0.001 |
| cubic5 (relaxed, arXiv 2606.00371) | band [0.7, 1.3], τ(1e-3)=0.12 | 10 | ≈0.004 |
| **cubic5b05 (our default)** | same generator, l0=0.05 | 10 | ≈0.010 |
| cubic5b10 / b002 | l0=0.1 / l0=0.002 (7 steps) | 10 / 14 | ≈0.02 / ≈5e-4 |
...
- **MoE probe platform**: 1080M params — hidden 768, 8 layers + 1 MTP block
  ...
  packing, micro-batch 12 × accumulation 2 (24,576 tokens per optimizer
...
- **Tiers**: screening = 500 micro-steps (≈14 min); confirmation = 2000
  micro-steps (≈45 min). Every quality claim is a same-seed (1337),
  same-data-order **paired** comparison. Paired noise floor: ±0.05 nat
  (Metal nondeterminism only). Cross-seed floor: σ = 0.11 nat (3 seeds:
...
- **Snapshot tooling** (`trainer/snapshot.py`; env-gated, zero overhead
  ...
  the 2×accum independent micro-batch gradients forming consecutive
  updates, with structured subsampling for stacked expert tensors.
  Analysis: `experiments/mp_fit.py`.

### 2.3 The measurement: direction persistence ρ(σ)
...
F-normalized momentum matrix, take its singular directions {u_i v_iᵀ} at a
...
from the same micro-batches; bin by σ. Persistent directions — signal, by
definition: the gradient keeps returning to them — have ρ ≈ 1; fresh-noise
directions have ρ ≈ 0. The boundary σ* is the collapse point of ρ(σ).

The estimator is validated end-to-end on synthetic spiked-MP ground truth:
bulk ρ ≈ 0, spike ρ = 0.91 against the BGN-theoretical 0.87–0.96, and the
recovered bulk edge λ̂₊ within 2% of truth. Nothing in the estimator assumes
...
Let Φ: ℝ^{m×n} → ℝ^{m×n} act on momentum. If Φ is **equivariant**
(Φ(QMR) = Q·Φ(M)·R for all orthogonal Q, R) and **stateless** (depends only
on the current M), then by von-Neumann-type representation theorems Φ is a
joint spectral map: Φ(M) = U·diag(τ)·Vᵀ with τ a function of the spectrum.
...
restricts itself to *separable polynomial* maps τ_i = p(σ_i), a proper
...
inside and asks: what is the optimal τ?
...
energy). EMA with decay β averages noise down by the effective sample size
ESS = (1+β)/(1−β) (39 at β=0.95) while signal stacks coherently.
Random-matrix theory (Marchenko–Pastur; BBP; Benaych-Georges–Nadakuditi)
then gives: a noise bulk with upper edge **λ₊ = √w·(1/√m + 1/√n)**;
...
the predicted edge (0.757 measured vs. 0.760 theory at σ/edge = 1.27; 0.10
at σ/edge = 0.98).
...
give the same shape: τ_i = cosθ_u(σ_i)·cosθ_v(σ_i). Hence the closed form
**t*(σ)**: zero below λ₊; partial weight just above it (a direction that is
mostly noise gets exactly its overlap fraction); →1 for σ ≫ λ₊.
...
hard knee + flat band is exactly "spectral-norm ball ∩ nuclear-norm
...
(τ_GD = ŝ·cosθ_u·cosθ_v); our target is the *whitened* signal (drop ŝ). To
...
powers σ^p. t* is unaffected: it tends to a constant at large σ — precisely
...
single-shot polynomial approximating a knee at ℓ needs degree ~1/√(smoothing
radius): measured, a smoothed t* with knee at 0.029 still has 0.33 error
and 0.32 noise leakage at deg-8 (10 GEMMs). Iterated composition grows
effective degree exponentially (k cubic steps ≡ degree 3^k), cost
~log(1/ℓ). The cubic iteration machinery is therefore the right substrate;
the theory re-prices only its l0 constant — and higher knees need *fewer*
steps (l0 = 0.03–0.09 ⇒ 3–4 steps, 6–8 GEMMs vs. 10). Cost and quality
...
- **Spike peeling diverges** (ŵ → 0) for almost every shape group. (An
  earlier draft cited a KS D = 0.60 against an MP bulk for the one
  nominally fittable group, 768×768; that statistic was numerically
  invalid — uniform-grid CDF integration diverges under the c = 1 MP
  density singularity at y = 0, reporting D ≈ 0.60 even on synthetic
  pure-MP data. The peeling divergence stands on its own.)
- **Spectra are smooth heavy-tailed** — Zipf slopes −0.7…−2.7 in
  log σ–log rank, no plateau bulk, no low-rank spike structure.
- **Entry noise is strongly anisotropic** — centered-residual row/column
  variance ratios of 10²–10⁴ (rows up to 3.2e4, columns up to 2.6e4),
  violating the i.i.d. premise behind λ₊ = √w(1/√m + 1/√n).
...
direction by how much of it is signal — which is exactly what ρ(σ)
measures, without a model.

### 4.2 ρ(σ) collapses sharply: the boundary σ* exists
...
| kv_up 128×768 | no collapse in view (ρ > 1 down to σ_min = 0.024) |

![Fig.1: ρ(σ) collapse and stationarity](figures/fig1_persistence.png)

σ* is *not* monotone in matrix size or aspect ratio: the within-MoE
variation (30×) is as large as the cross-architecture shift (§5.4). σ* must
...
A natural worry: σ* measured during warmup may expire later. Late-run
...
two lowest-σ* groups, the ones §8 turns on:
...
(Fractions are ρ minus the estimator floor; ≈1 = fully persistent.) Two
facts in this table drive everything downstream: **(i)** the collapse is
sharp in log σ, and stationary across micro-steps 300→1900; **(ii)** it is
*not* a step — the signal fraction climbs gradually (≈0.3 → 1.0 → 1.3)
across nearly a decade for the heavy-tailed groups. We call
[σ*, σ*_upper] the **transition band**.

**Operational band edges** (v3; replaces eyeballing). Bin per-direction
values by log σ (bins [1e-5, 3e-4, 1e-3, 3e-3, 1e-2, …], ×√10); per bin
take the *median* ρ and *median* floor (the median is essential — the
floor blows up at tiny σ); excess := median ρ − median floor.
σ*_upper := the lower edge of the first bin with excess ≥ 0.9;
σ* := the lower edge of the first bin with excess ≥ 0.3. Recomputed over
...
| shape group | σ* | σ*_upper (steps 500/750/950) |
|---|---|---|
...
| o_proj 768×384 / 384×768 | ≈0.003 | ≈3e-3, noisy at later steps |
| KDA gates 96×768 / 768×96 | 0.001–0.003 | no crossing ≤ 3e-2 (excess ≲ 0.5) |
| routed experts gu/dw | 0.003–0.01 | no crossing ≤ 1e-1 (excess ≲ 0.5) |
| kv_up 128×768 | no collapse in view | — |

So the band is narrow for the heavy-tailed low-σ* groups (upper edge
≈1–3×σ*) and wide for attention and the expert stacks (upper edge ≳10×σ*;
...
is the parameter-dominant attention block, whose upper edge is ≈1e-2.
...
baseline by ≈0.04 nat, so pre-fix and post-fix numbers are never subtracted
across frames.
...
**Definitions.** For each shape group: σ* := the collapse point of ρ(σ)
(§2.3); transition band := [σ*, σ*_upper] under the operational thresholds
of §4.3 (first bin with excess ≥ 0.3 / ≥ 0.9); knee50 := the σ receiving
half weight under a given spectral response.

> **Persistence-boundary law (corrected form).** Give zero weight below
> σ*; inside the transition band, shrink by the measured signal fraction;
> flatten above σ*_upper. A single-knee schedule should place knee50 at the
> **upper** edge σ*_upper — not at σ*. Deviations are penalized on both
> sides, and the two arms of the resulting U-shaped quality curve span
> exactly the band: knees at σ* hand 30–70%-noise directions full weight
> (variance injection); knees past σ*_upper under-whiten fully persistent
> directions.

The naive reading (binary: zero below σ*, flat above) is what §8 falsifies;
...
b05 = 5.264, paired noise floor ±0.05:

| probe | knee50 | loss@500 | Δ vs b05 | reading |
|---|---|---|---|---|
| classic NS5 (P31) | ≈0.001 | 5.358 | **+0.094** | far left arm |
| cubic5b002 (7 steps, P20) | ≈5e-4 (at σ*) | 5.318 | **+0.054** | left arm: transition band at full weight |
| cubic5 (P32) | ≈0.004 | 5.274 | +0.010 | left shoulder |
| **cubic5b05 (default, P19)** | ≈0.010 (≈σ*_upper) | **5.264** | — | **peak** |
| cubic5b10 (P33) | ≈0.02 | 5.349 | **+0.085** | right arm |
| PE5 (sharp knee) †pre-fix | ≲0.001 | +0.14 vs. classic | | far left; sharper is worse |
...
Per-seed Δ vs. b05 — b002: +0.054 / +0.014 / +0.024 (seeds 1337/1338/1339);
b10: +0.085 / +0.065 / +0.038. The arm penalties are positive on **every**
seed; cross-seed σ of the Δ is ≈0.02 nat for both arms. The monotone trio
τ(1e-3) = 0.86 / 0.47 / 0.12 → +0.14 / 0 / −0.08 (PE5 / classic / cubic5)
was the original smoking gun.

**Arms = band width.** Left arm: a knee at σ* ≈ 5e-4 hands the band
[σ*, ≈3e-3] — measured 30–70% non-persistent for the heavy-tailed groups —
full weight. Right arm: a knee at 0.02 > σ*_upper under-whitens the range
[3e-3, 1e-2] whose measured signal fraction is ≈1. The peak sits between,
at the upper edge of the parameter-dominant attention band (§4.3). This is
the measurable version of t*'s shape intuition.

![Fig.2: U-curve on two architectures](figures/fig2_ucurve.png)

### 5.3 Prediction scorecard (pre-registered in the theory note)

| # | prediction | verdict |
|---|---|---|
| 1 | optimal knee ≈ λ₊ closed form; raising the knee helps up to λ₊ | **half**: raising helps up to b05 then U-turns (b10 +0.09) ✓; the closed-form calibration fails (§4.1) ✗ — replaced by measured σ* |
| 2 | direction correlation collapses at λ̂₊ | **form right, position corrected**: ρ(σ) collapses (sharper than predicted), but at σ* ≠ λ̂₊ |
| 3 | per-shape knee scales as (1/√m + 1/√n) | **not confirmed**: measured σ* does not follow MP ratios (§4.2); per-head compensation is unnecessary post-fix (§6) |
| 4 | knee scales as √B with batch size | **not confirmed** at B/2 (P22): b07 vs. b05 within noise; the lever may be ESS-dominated at fixed β |
...
frame (U-shape, knee = persistence boundary, zero weight below σ*) did.
...
±0.05; five-point knee grid, all in the post-fix frame):

| schedule | knee50 | loss@500 | reading |
|---|---|---|---|
| b002 (P26c) | ≈5e-4 | **3.943** | knee at the dense σ* — plateau edge |
| b005 (P35) | ≈2e-3 | 3.950 | plateau |
| b01 (P34) | ≈5e-3 | 3.963 | plateau, drifting up |
...
appeared even with the knee at σ* itself. Measurement explains the shift
before any curve-fitting (probe_p27_dense_snap): dense σ* ≈ 3e-4–1e-3
(attention), ≤0.003 (FFN) — an order of magnitude *below* the MoE's
0.003–0.01. What does **not** move is the band's upper region: under the
§4.3 rule the dense FFN's excess reaches 0.9 only at ≈3e-2 and the dense
attention block never crosses 0.9 in view (0.5–0.87 across
[3e-3, 3e-1)) — the dense band is *wide*, overlapping the MoE's
1e-2–3e-2 region. So what the 10× shift moves is the band's **lower** edge.
...
−0.054), then spikes to 8.36 at step ≈600, half-recovers, and is
irreversibly NaN by step ≈1000 (the NaN guard skips non-finite updates but
...
steps (the plateau reaches all the way down to σ*), decisive by 2000 (b005
...
higher — on dense at knee50 ≈ 0.01, the same value as the MoE default,
...
never at σ\***. (Single seed and one run per point at this tier; the b002
divergence is one event, §10.)

## 6. Scaling and robustness checks

![Fig.3: shape / batch / per-head](figures/fig3_scaling.png)

- **Shape axis** (scorecard row 3). Per-head NS (64×768 blocks instead of
  768×768) with MP-motivated knee compensation ×1.9 (P21: 5.202, −0.06) and
  ...
  supported; per-group σ* measurement is the operative replacement.
- **Batch axis.** Halving the effective batch (bs6 × accum2), b07
  (= b05·√2) is not better than b05 (+0.02, within noise; P22). The √B
  prediction is unconfirmed at B/2; at fixed β the noise level may be
  ESS-dominated rather than batch-dominated.
- **Seeds.** Cross-seed σ of absolute loss = 0.11 nat (P25); cross-seed σ
  of the *paired* arm-vs-peak Δ is ≈0.02 nat, and both arm penalties are
  positive on all three seeds (§5.2). All ≤0.09-nat claims at the 500-step
  tier are *paired* claims (±0.05 floor); we claim the U's direction, not
...
learning rate — second-order harmless.** The measured σ* table (§4.2) turns
this into per-case arithmetic:

| observation | account |
|---|---|
| PE5 +0.14 (τ(1e-3) = 0.86) | σ = 1e-3 is below every measured attention boundary: a non-persistent direction at 0.86 weight — maximal injection |
| classic NS5 (τ(1e-3) = 0.47) | the same direction at half weight — between PE5 and cubic5, as observed |
| cubic5 −0.08 (τ(1e-3) = 0.12) | least injection of the trio; three points monotone in the predicted direction |
| no-NS, norm-only +0.34 | τ ∝ σ: top directions monopolize the step; mid-strength *persistent* directions underweighted — the opposite failure (under-whitening) |
| stale polar factor +0.4…0.5, worst in warmup | full weight on directions that have *rotated away* — zero-overlap full weight in the extreme; momentum rotates fastest in warmup ⇒ maximal harm there ✓ |
| per-head NS +0.14 (pre-fix stack only) | cutting to 64×768 raises the block's noise edge ≈1.8× (MP estimate), doubling injected noise mass at fixed coefficients. Not reproduced post-fix (§6) — consistent, not confirmed. Also matches Pion's report that per-head helps only combined with a high-pass |
...
at the warmup tier (§5.2→§5.4), and predicted the localization of
...
![Fig.4: Pareto + 2000-step reversal](figures/fig4_pareto_reversal.png)

The naive engineering reading — set each shape group's knee at its measured
σ* (l0 = 4.5σ*, so knee50 ≈ σ*), let the schedule generator pick its own
...
- **2000-step tier**: monotone reversal. Δloss crosses zero at step ≈800
...
- **Single-variable attribution** (2000 steps, same seed): per-head alone
  is neutral throughout (|Δ| ≤ 0.09; MTP slightly better, −0.03…−0.09);
...
- **Mechanism discrimination** (P30, §4.3): σ* did not drift, so staleness
  is not the cause; the binary rule is. The lowest-knee groups have the
  widest transition bands (heaviest tails, Zipf ≈ −2.4): "flatten to 1
  above σ*" hands a near-decade of 30–70%-noise directions full weight. The
...
edge (which for these groups is ≈b05's position: the status quo), or
...
exactly where the corrected law says it must (lowest σ*, widest band, most
...
- **Spectral shaping in Muon**: classic NS5 (Jordan et al.); Polar Express
  ...
  Pion (arXiv 2605.19282) — promotion/suppression split; tail suppression ≈
...
- **Outside the equivariant/stateless class**: NorMuon (arXiv 2510.05491;
  ...
  (arXiv 2603.09697; curvature). The σ* measurement is orthogonal to and
...
- **Optimizer-benchmark evidence**: the nanoGPT optimizer track's
  decomposable gain stack (Muon → Muon² → NorMuonH → u/w-floor → radial
  brake → RowFloor + cautious WD; 3600 → 2690 steps) corroborates that
...
- **Scale and window.** All probes ≤2000 micro-steps, 132M/1080M models,
  one codebase, single node, bf16. Stationarity of σ* is established for
  micro-steps 300–1900 only; behavior at 100k+ steps, other β, and much
  larger batches is unknown. The √B law failing at B/2 does not bound B/8.
- **Frames.** The gate-fix of P19 moved all absolute losses (~0.04 nat);
  ...
  as qualitative extremal markers (†) and cross-frame Δs are never taken.
...
- **Estimator.** The i.i.d. reading of the ρ chance floor is approximate
...
- **Unmeasured region.** kv_up 128×768 shows no collapse in view (ρ > 1 at
  σ_min = 0.024); the law is silent there beyond "flatten". Dense-band
  ...
  re-measured.
- **Open mechanism.** The 10× dense-vs-MoE shift of the band's *lower* edge
  is measured, not explained; the upper region (≈1e-2–3e-2, excess never
  quite reaching 0.9 for attention) is common to both architectures, and
  why the steady-state optimum lands at knee50 ≈ 0.01 on both is
...
persistence boundary σ* and its transition band. The boundary is read off
...
ρ(σ) before touching the knee; place the half-weight point at the band's
...
- Probe → artifact map: P13 cubic5 · P14/15 frac family · P16 b05 · P17 b10
  · P18 snapshots (optimizer steps 150/300) · P19 gate-fix baseline · P20
  b002 · P21/P21b per-head ± compensation · P22 batch-half · P23 EdgeCubic
  · P24 per-head+edge · P25 seeds · P26 dense knees (a: b05, b: b10, c:
  b002) · P27 dense snapshots · P28 2000-step confirmation (a: baseline, b:
  per-head+edge) · P29 single-variable attribution (a: per-head, b: edge) ·
  P30 late snapshots (optimizer steps 500/750/950) · P31–P33 post-fix U
  re-runs (classic / cubic5 / b10) · P34/P35 dense b01/b005 · P36–P39 arm
  seeds (b002/b10 × 1338/1339) · P40–P42 dense 2000-step tier (b002
  diverged / b05 / b005) · P43 MoE b10 2000-step check (vs. P28a). Logs in
...
- Code: `trainer/muon.py` (schedules `_CUBIC5B*_COEFFS`, `_cubic_schedule`,
  ...
  edge, §4.3), `test_muonh_zeroinit.py` (§4.4 regression). The ρ(σ)
...
- **Fig.1** (existence + stationarity): ρ(σ) per shape group, log σ; curves
...
- **Fig.2** (money): left — MoE Δloss@500 vs. knee50, five post-fix points
  solid (single frame), pre-fix points hollow (qualitative only), shaded
  transition band, 3-seed Δ error bars on the two arms; right — dense 132M
  five-point grid at the 500-step tier with its measured band (σ* 10×
  lower, upper region ≈3e-2 shared with the MoE). Data: §5.2/§5.4 tables.
- **Fig.3** (scaling triple): per-group σ* (log bars); batch-half pair;
  per-head ± compensation. Data: §4.2 table, P22, P21/P21b.
- **Fig.4** (Pareto + reversal): 500-step loss vs. step/s for
  b05 / EdgeCubic / per-head+{b05, edge}; 2000-step Δloss (solid) and ΔMTP
